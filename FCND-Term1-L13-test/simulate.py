...
def zy_flight(z_traj, 
	...
	phi_g_path, phi_g_dot_path, phi_g_dot_dot_path = phi_g_traj
	...
	drone.X = np.array([z_path[0],
						y_path[0],
						# math.atan2(y_dot_path[0],z_dot_path[0]),
						phi_g_path[0]/drone.g,
						z_dot_path[0],
						y_dot_path[0],
						phi_g_dot_path[0]/drone.g])
	...
	for i in range(0,z_path.shape[0]-1):
		...
		for _ in range(inner_loop_speed_up):
			u_2 = controller.attitude_controller(phi_g_path[i]/drone.g, 
												 # phi_commanded, 
													 drone.X[2],
													 drone.X[5],
													 phi_g_dot_path[i]/drone.g, 
													 phi_g_dot_dot_path[i]/drone.g
													 )
